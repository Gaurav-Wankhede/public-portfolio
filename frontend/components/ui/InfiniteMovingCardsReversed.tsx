...
interface InfiniteMovingCardsReversedProps {
  items: {
    quote: string;
    name: string;
    image_url: string;
...
export const InfiniteMovingCardsReversed: React.FC<
  InfiniteMovingCardsReversedProps
> = ({
  items,
  direction = "right",
  pauseOnHover = true,
      ...
      >
        {duplicatedItems.map((item, index) => (
          <li
            ...
            key={`reversed-testimonial-${item.name.replace(/\s+/g, "-")}-${index}`}
          >
            {/* Testimonial Content - 65% width on the left */}
            ...
            {/* Profile Section - 35% width on the right */}
            <div
              className={cn(
                "w-[35%] flex flex-col justify-center items-center p-4 border-l",
                theme === "dark"
